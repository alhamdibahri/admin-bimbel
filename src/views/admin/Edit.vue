...
export default {
  ...
  methods: {
    ...
    getData(){
        ...
        this.axios.get('users/edit/' + this.$route.params.id, config)
        .then((response) => {
            let values = response.data.values
            this.nama = values.nama
            this.username = values.username
            this.email = values.email
            this.role = values.role_id
            this.password = values.password
            this.no_handphone = values.no_handphone
            this.foto_get = values.foto
            this.provinsi = values.province_id
            if(this.provinsi != null) this.getKabupaten()
            this.kabupaten = values.regency_id
            if(this.kabupaten != null) this.getKecamatan()
            this.kecamatan = values.district_id
            if(this.kecamatan != null)this.getDesa()
            this.desa = values.village_id
            this.agama = values.religion_id
            this.jenis_kelamin = values.gender_id
            this.tempat_lahir = values.tempat_lahir
            this.tanggal_lahir = values.tanggal_lahir
            this.alamat = values.alamat
            this.no_handphone = values.no_handphone
            this.foto_get = values.foto
          })
          .catch(error => {
    ...
    save(){
        ...
        this.axios.patch('admin/update', formData, config)
        .then((response) => {
            let message  = response.data.message
            this.$router.push({name: 'Data Admin', params: {foo: message}})
          })
          .catch(error => {
