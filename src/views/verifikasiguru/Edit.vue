<template>
    ...
    <CRow>
      <CCol sm="12">
        <CCard>
          <CCardHeader>
            <i class="fa fa-edit"></i> EDIT DATA GURU
          ...
          <CCardBody>
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-10">
                <Select2 v-model="kabupaten" :options="valueKabupaten"  @change="getKecamatan" :settings="{ placeholder: '-Pilih Kabupaten/Kota-', width: '100%' }"
                    />
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-10">
                <Select2 v-model="pendidikan" :options="valuePendidikan"  :settings="{ placeholder: '-Pilih Pendidikan Terakhir-', width: '100%' }"
                    />
                <div v-if="errors.get('pendidikan')" style="color:red;">
                  <small>{{ errors.get('pendidikan') }}</small>
                </div>
              </div>
            </div>
            <div class="form-group row">
              ...
              <div class="col-sm-10">
                <Select2 @change="getKelas" v-model="kategori_kelas" :options="valueKategori"  :settings="{ placeholder: '-Pilih Kategori Kelas-', width: '100%' }"
                    />
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-10">
                <Select2 @change="getPelajaran" v-model="kelas" :options="valueKelas"  :settings="{ placeholder: '-Pilih Kelas-', width: '100%' }"
                    />
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-10">
                <Select2 v-model="mata_pelajaran" :options="valuePelajaran"  :settings="{ placeholder: '-Pilih Mata Pelajaran-', width: '100%' }"
                    />
                <div v-if="errors.get('mata_pelajaran')" style="color:red;">
                  <small>{{ errors.get('mata_pelajaran') }}</small>
                </div>
              </div>
            </div>
            <div class="form-group row">
              <label for="staticEmail" class="col-sm-2 col-form-label">Jenis Harga</label>
              <div class="col-sm-10">
                <Select2 v-model="jenis_harga" :options="valueJenisHarga"  :settings="{ placeholder: '-Pilih Jenis Harga-', width: '100%' }"
                    />
                <div v-if="errors.get('jenis_harga')" style="color:red;">
                  <small>{{ errors.get('jenis_harga') }}</small>
                </div>
              </div>
            </div>
            <div class="form-group row">
              ...
              <div class="col-sm-10">
                <input type="text" class="form-control" placeholder="Harga" v-model="harga">
                <div v-if="errors.get('harga')" style="color:red;">
                    <small>{{ errors.get('harga') }}</small>
                </div>
              </div>
            </div>
            <div class="form-group row">
...
export default {
  ...
  data () {
    return {
        urlFoto: 'http://192.168.43.206:8081/foto_user/',
        nama: '',
        username: '',
        role: '',
        provinsi: '',
  ...
  methods: {
    ...
    getPelajaran(){
        ...
        this.axios.get('ddlpelajaran/' + this.kelas, config)
        .then((response) => {
            let values = response.data.values
            this.valuePelajaran = []
            for(let x = 0; x<values.length; x++){
              this.valuePelajaran.push({id: values[x].id_mata_pelajaran, text: values[x].mata_pelajaran})
            }
          })
          .catch(error => {
            this.$notify({
    ...
    getEducations(){
        ...
        this.axios.get('ddleducation', config)
        .then((response) => {
            let values = response.data.values
            for(let x = 0; x<values.length; x++){
              this.valuePendidikan.push({id: values[x].id_education, text: values[x].education})
            }
          })
          .catch(error => {
            this.$notify({
    ...
    getJenisHarga(){
      let config = {
        ...
        this.axios.get('ddljenisharga', config)
        .then((response) => {
            let values = response.data.values
            for(let x = 0; x<values.length; x++){
              this.valueJenisHarga.push({id: values[x].id_jenis_harga, text: values[x].jenis_harga})
            }
          })
          .catch(error => {
            this.errors.record(error.response.data)
    ...
    getData(){
        ...
        this.axios.get('guru/edit/' + this.$route.params.id, config)
        .then((response) => {
            let values = response.data.values
            ...
            this.no_handphone = values[0].no_handphone
            this.foto_get = values[0].foto
            this.kategori_kelas = values[0].guru.kategori_kelas_id
            if(this.kategori_kelas != null)this.getKelas()
            this.kelas = values[0].guru.kelas_id
            if(this.kelas != null) this.getPelajaran()
            this.mata_pelajaran = values[0].guru.mata_pelajaran_id
            this.pendidikan = values[0].guru.education_id
            this.jenis_harga = values[0].guru.jenis_harga_id
            this.harga =  values[0].guru.harga
          })
          .catch(error => {
            this.$notify({
              group: 'foo',
    ...
    save(){
        ...
        this.axios.patch('guru/update', formData, config)
        .then((response) => {
            let message  = response.data.message
            this.$router.push({name: 'Data Guru', params: {foo: message}})
          })
          .catch(error => {
            this.errors.record(error.response.data)
            this.$notify({
